...
class mainwindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, myconnection: psycopg2.extensions.connection, mycursor: psycopg2.extensions.cursor) -> None:
        ...
        self.ui.book_table.setHorizontalHeaderLabels(
            (
                "Фамилия",
                "Имя",
                "Отчество",
        ...
        self.lines: list[QtWidgets.QLineEdit] = (
            self.ui.last_name_line,
            self.ui.name_line,
            self.ui.middle_name_line,
            self.ui.street_line,
            self.ui.house_line,
            self.ui.korp_line,
            self.ui.apart_line,
    ...
    def __populate(self) -> None:
        ...
        self.mycursor.execute(
            """SELECT family.f_val, n_val, o_val, s_val, house, korp, apart, mob
    ...
    def __get_fkey_values(self, data: list[str]) -> list[int]:
        numbers: list[int] = []
        values: list[str] = [data[1], data[2], data[3], data[4]]
        for i, parent_table in enumerate(PARENT_TABLES):
            self.mycursor.execute(
                f"SELECT {parent_table[1]} FROM {parent_table[0]} WHERE {parent_table[2]} = '{values[i]}';"
            )
            ...
            if self.mycursor.fetchall():
                ...
                self.mycursor.execute(
                    f"""INSERT INTO {parent_table[0]} ({parent_table[1]}, {parent_table[2]}) VALUES 
                                        (DEFAULT, '{values[i]}')"""
                )
                self.myconnection.commit()
                self.mycursor.execute(
                    f"SELECT last_value FROM {parent_table[0]}_{parent_table[1]}_seq;"
                )
                numbers.append(self.mycursor.fetchall()[0][0])
        return numbers
    ...
    def __add_entry(self) -> None:
        ...
        SANITIZE INPUTS
        ADD CLIENTSIDE TYPECHECKING
        ADD SUPPORT FOR INCOMPLETE ADDITION
        SWAP TO COMBO BOXES

        """
        self.ui.book_table.itemChanged.disconnect()
        new_data = (
            self.ui.book_table.rowCount() + 1,
            self.ui.last_name_line.text(),
            self.ui.name_line.text(),
            self.ui.middle_name_line.text(),
            self.ui.street_line.text(),
            self.ui.house_line.text(),
            self.ui.korp_line.text(),
            int(self.ui.apart_line.text()),
            self.ui.phone_line.text(),
        )
        fkey_values: list[int, int, int, int] = self.__get_fkey_values(new_data)
        self.mycursor.execute(
            f"""INSERT INTO {BOOK_TABLE[0]} ({BOOK_TABLE[1]}, {BOOK_TABLE[2]}, {BOOK_TABLE[3]}, 
    ...
    def __get_specific_fkey(self, data: list[str]) -> int:
        ...
        self.mycursor.execute(
            f"SELECT {parent_table[1]} FROM {parent_table[0]} WHERE {parent_table[2]} = '{data[2]}';"
        )
        print(self.mycursor.fetchall())
        if self.mycursor.fetchall():
    ...
    def __update_entry(self, item: QTableWidgetItem) -> None:
        ...
        self.mycursor.execute(
            f"""UPDATE {BOOK_TABLE[0]} SET {BOOK_TABLE[new_data[0]+2]} = {"" if new_data[0]==7 else "'"}{new_data[2]}{"" if new_data[0]==7 else "'"}
    ...
    def __deletion_mode(self) -> None:
        ...
        ADD A CONFIRMATION OF INTENT
        """
        def __delete_item(window: mainwindow, item: QTableWidgetItem) -> None:
            req_id: int = item.row()
            window.mycursor.execute(
                f"DELETE FROM {BOOK_TABLE[0]} WHERE {BOOK_TABLE[1]} = {req_id}"
            )
    ...
    def __update_table(self, query_result: list[tuple[Any]]) -> None:
        ...
        for i, entry in enumerate(query_result):
            for j, point in enumerate(entry):
                self.ui.book_table.setItem(i, j, QTableWidgetItem(a(point)))
                self.ui.book_table.item(i, j).setTextAlignment(
    ...
    def __perform_search(self) -> None:
        """
        TO-DO:
        COMVERT QUERY CREATION TO A LOOP
        """
        self.ui.book_table.itemChanged.disconnect()
    ...
    def __add_to_parents(self) -> None:
        for i in range(3):
            if self.lines[i].text():
                self.mycursor.execute(
                    f"""INSERT INTO {PARENT_TABLES[i][0]} ({PARENT_TABLES[i][1]}, {PARENT_TABLES[i][2]}) VALUES
    ...
    def __update_in_parents(self) -> None:
        ...
        def get_old_text(item: QTableWidgetItem) -> None:
            nonlocal old_text
            old_text = item.text()
        ...
        def update_parents(window: mainwindow, item: QTableWidgetItem) -> None:
            ...
            self.mycursor.execute(
                f"""UPDATE {PARENT_TABLES[column][0]} SET {PARENT_TABLES[column][2]} = 
